...
/**
 * Created with IntelliJ IDEA. User: aykut Date: 11/13/13 Time: 9:44 AM To change this template use File | Settings | File Templates.
 */
public class BuzzJapaneseTokenizer extends BuzzTokenizer {
    ...
    private static final String notWord = "ない";

    private static final com.atilika.kuromoji.ipadic.Tokenizer tokenizer = new Tokenizer();
    ...
    private static String transformSentenceBeforeTokenization(String sentence) {
        ...
        sentence = doReplace(easternEmoticonsPositivePattern, sentence, " chmodelpositivesmiley ");
        sentence = doReplace(easternEmoticonsNegativePattern, sentence, " chmodelnegativesmiley ");
        ...
        sentence = sentence.replaceAll(remainingPunctuationRegEx, " chmodelpunctuation ");
    ...
    private IntList negateSpellStem(List<String> tokens) {
        ...
        if (tokens.size() == 1) {
            if (getVocabulary().containsKey(tokens.get(0))) {
                transformedList.add(getVocabulary().getInt(tokens.get(0)));
            }
        }
        while (i < tokens.size() - 1) {
            String s1 = tokens.get(i);
            String s2 = tokens.get(i + 1);
            if (s2.equals(notWord)) {
                s1 = s1 + "_not";
    ...
    public IntList tokenIndices(String sentence) {
        return tokenIndices(sentence, BuzzJapaneseTokenizer::tokenize);
    }

    @Override
    public IntList tokenIndices(String string, Function<String, List<String>> tokenizer) {
        return negateSpellStem(tokenizer.apply(string).stream().map(BuzzTokenizer::replaceRepeatedCharacters).collect(Collectors.toList()));
    }
